...
export type AllSanitySchemaTypes = SanityImagePaletteSwatch | SanityImagePalette | SanityImageDimensions | SanityFileAsset | Geopoint | NewsType | Order | Product | Category | Slug | Hero | BlockContent | SanityImageCrop | SanityImageHotspot | SanityImageAsset | SanityAssetSourceData | SanityImageMetadata;
...
// Variable: POPULAR_CATEGORIES_QUERY
// Query: *[ _type == "category" ][0..3]
export type POPULAR_CATEGORIES_QUERYResult = Array<{
  _id: string;
  _type: "category";
  _createdAt: string;
  _updatedAt: string;
...
// Query: *[      _type == "product" &&      (name match $query || description[].children[].text match $query || $query == "") &&      ($gender == "" || gender == $gender) &&      ($sizes == null || defined(*[_type == "product" && _id == ^._id && count(sizes[size in $sizes]) > 0][0])) &&      ($categories == null || count($categories) == 0 || *[_type=="category" && slug.current in $categories]._id  match categories[]._ref) &&      price >= $minPrice &&      price <= $maxPrice     ]    | order(name asc)
...
declare module "@sanity/client" {
  interface SanityQueries {
    "*[ _type == \"category\" ]": ALL_CATEGORIES_QUERYResult;
    "*[ _type == \"category\" ][0..3] ": POPULAR_CATEGORIES_QUERYResult;
